    ...
    private function subJoin(): QueryBuilder
    {
        ...
        return DB::table($this->prefix.'sessions_rollup_1day')
            ...
            ->select([
                ...
                'object_uid',
                DB::raw("
                    COALESCE({$this->aggregate}(CASE WHEN day BETWEEN '{$currentStart}' AND '{$currentEnd}' THEN session_count END), 0) -
                    COALESCE({$this->aggregate}(CASE WHEN day BETWEEN '{$previousStart}' AND '{$previousEnd}' THEN session_count END), 0) as delta
                "),
            ]);
    }
