...
test('Creates correct sql for text', function () {
    ...
    expect($builder->toSql())->toBe(
        sprintf('select * from "users" where a_column::text between ? and ?', $prefix)
    );
...
test('Creates correct sql for number', function () {
    ...
    expect($builder->toSql())->toBe(
        sprintf('select * from "users" where a_column::numeric between ? and ?', $prefix)
    );
...
test('Creates correct sql for date', function () {
    ...
    expect($builder->toSql())->toBe(
        sprintf('select * from "users" where a_column::date between ? and ?', $prefix)
    );
