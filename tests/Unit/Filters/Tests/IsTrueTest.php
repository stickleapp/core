...
test('Creates correct sql', function (): void {
    ...
    expect($builder->toSql())->toBe(
        sprintf("select * from \"users\" where (data->'a_column')::boolean = true", $prefix)
    );
});

test('Joins with or when the operator is or', function (): void {

    $filter = Filter::boolean('a_column')->isTrue();
    ...
    expect($builder->toSql())->toBe(
        "select * from \"users\" where \"id\" > ? or (data->'a_column')::boolean = true"
    );
});
